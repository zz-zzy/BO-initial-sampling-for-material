...
class SampleYDistance(Acquisition):
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        ...
        # 复制 y_all 的列 num_rows 次
        expanded_y_all = np.tile(y_all, (1, num_rows))

        # 转置矩阵
        transposed_expanded_y_all = np.transpose(expanded_y_all)

        # 计算差异
        differences = transposed_expanded_y_all - Y

        # 取绝对值
        absolute_differences = np.abs(differences)

        # 找到每一列的最小值
        min_values_per_column = np.min(absolute_differences, axis=0)

        # 将结果重塑形状
        ydis_acquisition = min_values_per_column.reshape(-1, 1)

        return ydis_acquisition
...
class MeanPluginSampleYDistance(SampleYDistance):
...
class MultipointSampleYDistance(SampleYDistance):
    def __init__(
        self,
        ...
        """
        super(MultipointSampleYDistance, self).__init__(model, jitter)
